...
#### Fixed
- **Related name conflicts**: Added explicit `related_name` values to `groups` and `user_permissions` fields in `BitcoinCashUser` model to prevent reverse accessor conflicts with Django's default `auth.User` model
...
- **Django**: `0.2.0` → `0.2.1`
- **FastAPI**: `0.2.0` → `0.2.1`
- **JavaScript Client**: `0.2.0` (unchanged)
