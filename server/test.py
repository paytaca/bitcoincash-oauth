"""
Test script for Bitcoin Cash OAuth Server
"""
...
def test_validator():
    ...
    print("Testing BitcoinCashValidator...")
    
    from validator import BitcoinCashValidator, public_key_to_address
    
    # Test address validation
    print("\n1. Testing address validation:")
    
    # This is a valid mainnet address (example)
    test_address = "1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
    is_valid, network = BitcoinCashValidator.validate_address(test_address)
    print(f"   Address: {test_address}")
    print(f"   Valid: {is_valid}, Network: {network}")
    
    # Invalid address
    invalid_address = "invalidaddress123"
    is_valid, network = BitcoinCashValidator.validate_address(invalid_address)
    print(f"   Invalid address test: {is_valid}")
    
    # Test public key to address conversion
    print("\n2. Testing public key to address conversion:")
    # Example compressed public key (33 bytes)
    example_pubkey = "0279BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798"
    try:
        address = public_key_to_address(example_pubkey, "mainnet")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Bitcoin Cash OAuth Server Tests")
    print("=" * 60)
    ...
    try:
        ...
        print("=" * 60)
        print("\nTo run the server:")
        print("  cd server && pip install -r requirements.txt && python main.py")
